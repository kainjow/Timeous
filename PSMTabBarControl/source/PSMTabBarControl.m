...
NSString *const PSMTabBarControlDidFinishDragNotification = @"PSMTabBarControlDidFinishDragNotification";
...
- (BOOL)performDragOperation:(id <NSDraggingInfo>)sender
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:PSMTabBarControlDidFinishDragNotification object:self];
    return YES;
}
...
{
	if([self closeAlert]){
		[[self closeAlert] beginSheetModalForWindow:[self window]
									  modalDelegate:self
...
- (void)tabViewDidChangeNumberOfTabViewItems:(NSTabView *)aTabView
{
    ...
    NSEnumerator *e = [[NSArray arrayWithArray:_cells] objectEnumerator];
    PSMTabBarCell *cell;
    while(cell = [e nextObject]){
...
{
	if (_closeAlert != closeAlert)
	{
		[_closeAlert release];
		_closeAlert = [closeAlert retain];
	}
}

- (void)closeAlertDidEnd:(NSAlert *)alert returnCode:(int)returnCode contextInfo:(void *)contextInfo
{
	id sender = contextInfo;
	...
	NSAlert *closeAlert = [[self closeAlert] retain];
	[self setCloseAlert:nil];
	[self closeTabClick:sender];
	[self setCloseAlert:closeAlert];
	[closeAlert release];
}
